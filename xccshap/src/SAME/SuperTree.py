...
class SuperNode():
    ...
    def predict(self,X,feature_names):
        def predict_datum(node,x,feature_names,depth):  
            if node.is_leaf:
                return depth, np.argmax(node.labels)
            else:
                if not node.feat is None:
                    Xf = np.where(feature_names==node.feat)
                ...
                return predict_datum(node.children[next_node],x,feature_names,depth+1)
        return np.array([predict_datum(self,el,feature_names,1)[0] for el in X]), np.array([predict_datum(self,el,feature_names,1)[1] for el in X])
