...
def ccshap_full(id_dataset, output_path, model_path, max_depth=100, model_type = XGBClassifier()):
   ...
   for target_col in df_y.columns:
      ...
      for rnd in range(10):
         ...
         pkl_file_name = "xgbta_model_"+dataset_name.replace(" ", "_")+"_"+target_col.replace(" ", "_")+"_"+str(rnd)+".pkl"
      ...
      output_file_name = "xccshap_var_xgbta_"+dataset_name.replace(" ", "_")+"_"+target_col.replace(" ", "_")+".csv"
...
def main(argv):
   ...
   for opt, arg in opts:
      if opt in ("-h", "--help"):
         ...
         id_dataset = int(arg)
   print("XGBTA: test using "+classifier+" on: "+str(id_dataset))
   if (classifier=='xgb'):
